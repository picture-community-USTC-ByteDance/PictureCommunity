...
func QueryUserDataByUserID(uid uint) (error, _response.UserData) {
	...
	err := global.MysqlDB.Model(db.UserData{}).
		...
		Where("uid=?", uid).Scan(&userdata).Error

	return err, userdata
}

// 在用户主页，查自己的帖子数组信息
func QueryUserPostsByUserID(uid uint, page int, pageSize int) (error, int64, []_response.UserPosts) {
	var userposts []_response.UserPosts
	var count int64
	// 在db.Post{}数据库中查询，查询结果保存到结构体变量userposts中
	err := global.MysqlDB.Model(db.Post{}).
		Select("pid,title_photo_url,like_number,comment_number,forward_number").
		Where("uid=?", uid).Count(&count).
		Offset((page - 1) * pageSize).Limit(pageSize).Scan(userposts).Error
	return err, count, userposts
}
